...
func (h *ExportDataHandler) ExportStockMovementsByYear(
	context *fiber.Ctx,
) error {
	yearString := context.Query("year")
	if yearString == "" {
...
func (h *ExportDataHandler) ExportAllStockMovements(
	context *fiber.Ctx,
) error {
	response, err := h.service.ExportStockMovements(nil)
	if err != nil {
		return utils.HandleError(context, err)
	}
...
func (h *ExportDataHandler) ExportCustomerDistributionByYear(
	context *fiber.Ctx,
) error {
	yearString := context.Query("year")
	if yearString == "" {
	...
	response, err := h.service.ExportCustomerDistribution(&year)
	if err != nil {
		return utils.HandleError(context, err)
	}
...
func (h *ExportDataHandler) ExportAllCustomerDistribution(
	context *fiber.Ctx,
) error {
	response, err := h.service.ExportCustomerDistribution(nil)
	if err != nil {
		return utils.HandleError(context, err)
	}
